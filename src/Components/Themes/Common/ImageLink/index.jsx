      ...
      {link?.redirect_link?.link_type === 'external_url' ? (
        <Link href={link?.redirect_link?.link || '/'} target='_blank'>
          <div className={`${classes?.customHoverClass ? classes?.customHoverClass : 'home-contain hover-effect'}`}>
            {ratioImage ? <RatioImage src={imgUrl} className={`bg-img ${customRatioClass}`} alt='banner' /> : imgUrl && <Image src={imgUrl} className={`img-fluid ${customRatioClass}`} alt='banner' height={height} width={width}/>}
          </div>
        </Link>
