...
const HeaderDealModal = ({ setModal, modal }) => {
  ...
  const { data:filteredProduct, refetch, isLoading } = useQuery([ProductAPI,"DealModal" ], () => request({ url: ProductAPI, params: { ...getProductIds, status: 1 } }), {enabled: false,refetchOnWindowFocus: false,select: (data) => data?.data?.data});
  useEffect(() => {
