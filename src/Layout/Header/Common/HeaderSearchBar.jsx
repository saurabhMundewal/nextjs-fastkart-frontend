...
const HeaderSearchBar = ({ResponsiveSearch,searchBarOpen,setSearchBarOpen ,style ,ClassicHeader}) => {
  ...
  const { data: categoryData, refetch, isLoading: categoryIsLoading } = useQuery(["CategoryAPIBaiscSearch"], () => request({ url: CategoryAPI, params: { status: 1, paginate : selectedValue === "All Category" ? 4:paginate , ids: selectedValue ? selectedValue === "All Category" ? null :selectedValue : null, search: categoryCustomSearch ? categoryCustomSearch : null, } }), { enabled: false, refetchOnWindowFocus: false, select: (data) => data.data.data });
  ...
  useEffect(() => { setSelectedItemIndex(null); setIsComponentVisible(false), setSearchValue(""); setSearchArray(searchList?.slice(0, 5)) }, [pathName])

  const onHandleSearch = () => {
    if (searchValue) { router.push(`/search?search=${searchValue}`); } else { router.push(`/search`); }
  };
  ...
  const handleArrowKey = (direction) => {
    if (searchArr.length > 0) {
      ...
      if (selectedItemElement) { selectedItemElement.scrollIntoView({ behavior: 'smooth', block: 'center', }); }
  ...
  return (
    <>
    {mobileSize && ResponsiveSearch ? <div className={`search-full ${searchBarOpen ? "open":""} `}>
      <InputGroup>
        <InputGroupText>
          <RiSearchLine className='font-light' />
        </InputGroupText>
        {/* <Input type='text' className='form-control search-type' placeholder='Search here..' /> */}
        <Input onClick={() => { setIsComponentVisible(true); setSearchArray(searchList?.slice(0, 5)) }} type='search' className='search-type' placeholder="Search..." value={searchValue}
              onKeyDown={(e) => {
                if (e.key === 'ArrowDown') { handleArrowKey(1); } else if (e.key === 'ArrowUp') { handleArrowKey(-1); } else if (e.key === 'Enter') { handleEnterKey(); }
              }} onChange={(e) => onChangeHandle(e.target.value)} />
        <InputGroupText className='close-search'>
          <RiCloseLine className='font-light' onClick={()=>{setSearchBarOpen && setSearchBarOpen(prev =>!prev)}} />
        </InputGroupText>
    ...
    {ClassicHeader  && ResponsiveSearch ? <div className={`search-full ${searchBarOpen ? "open":""} `}>
      <InputGroup>
        <InputGroupText>
          <RiSearchLine className='font-light' />
        </InputGroupText>
    ...
    {
       !mobileSize && style == "standard" ? <div className='middle-box'>
       <div className='center-box'>
         <div className='searchbar-box order-xl-1 d-none d-xl-block' >
           <Input onClick={()=>{ setIsComponentVisible(true); setSearchArray(searchList?.slice(0, 5)) } }  type='search'  placeholder="Search..." value={searchValue}
             onKeyDown={(e) => {
             if (e.key === 'ArrowDown') { handleArrowKey(1);} else if (e.key === 'ArrowUp') { handleArrowKey(-1); } else if (e.key === 'Enter') { handleEnterKey();}
           }} onChange={(e) => onChangeHandle(e.target.value)} />
           {isComponentVisible && <SearchDropDown ref={ref} selectedItemIndex={selectedItemIndex} searchArr={searchArr} categoryLoading={categoryIsLoading} categoryData={categoryData} searchValue={searchValue} />}
       ...
       {
       !mobileSize && style == "minimal" ? <div className='middle-box'>
       <div className='center-box'>
    ...
    {!mobileSize &&  style == 'basic' ? <div className='middle-box custom-search-box'>
      <div className="search-box">
        <div className="input-group">
          ...
          <div className='search-suggestion-wrap'  >
            <Input onClick={() => { setIsComponentVisible(true); setSearchArray(searchList?.slice(0, 5)) }} type='search' className='form-control' placeholder="Search..." value={searchValue}
              onKeyDown={(e) => {
                if (e.key === 'ArrowDown') { handleArrowKey(1); } else if (e.key === 'ArrowUp') { handleArrowKey(-1); } else if (e.key === 'Enter') { handleEnterKey(); }
              }} onChange={(e) => onChangeHandle(e.target.value)} />
